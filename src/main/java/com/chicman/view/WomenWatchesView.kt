...
@SpringComponent
@UIScope
class WomenWatchesView(private val eventHandler: ChicManUI) : VerticalLayout(),
    View {
    private val log =
        LoggerFactory.getLogger(WomenWatchesView::class.java)
    ...
    override fun enter(event: ViewChangeEvent) {
        log.info("enter Women Watches view with parameter /" + event.parameters)
        eventHandler.toolbar?.setMenuWomenWatchesActive()
    ...
    private fun init() {
        ...
        val btnWomenWatches =
            Button(Messages.get("page.women"))
        btnWomenWatches.isEnabled = false
        tracking.addTracking(btnWomenWatches)
    ...
    private fun changeSortItemCondition(sortCondition: String) {
        if (sortCondition == Messages.get("sort.popularity")) {
            sortItemByPopularity()
        } else if (sortCondition == Messages.get("sort.fresh")) {
            sortItemByFresh()
        } else if (sortCondition == Messages.get("sort.discount")) {
            sortItemByDiscount()
        } else if (sortCondition == Messages.get("sort.price.asc")) {
            sortItemByPriceAsc()
        } else if (sortCondition == Messages.get("sort.price.desc")) {
            sortItemByPriceDesc()
        }
    }
    ...
    private fun sortItemByPopularity() {
        ...
        productList = Utils.filterWomenWatchList(productList)
        comboSortList!!.value = Messages.get("sort.popularity")
        refreshProductItem()
    }
    ...
    private fun sortItemByDiscount() {
        productList = eventHandler.productsRepo
